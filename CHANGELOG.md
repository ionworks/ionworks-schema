...
### Added
- `TimeRangeSpec` accepts `duration` as an alternative to `end`, measured from
  `start`: `TimeRangeSpec(start=0.0, duration=86_400.0)` is the first day of a
  test. It resolves to `end` at construction, so the emitted config still
  carries only `start` and `end`. Passing both `end` and `duration`, or a
  negative `duration`, is rejected.
- `TimeRangeSpec` is exported from the top-level `ionworks_schema` package.

## [0.28.0] - 2026-09-24

### Added
- `AnalysisSpec`, a selector (`analysis_id`, `analysis_type`, `name`) for one
  of a measurement's stored analyses. A data input of
  `{"data": "db:<measurement_id>", "analysis": {...}}`, or an
  `ionworksdata.AnalysisLoader`, reads that analysis as an objective's data
  instead of the measurement's time series, so a `CycleAgeing` fit can take its
  targets from the measurement it is fitted against. `analysis` cannot be
  combined with `options` or `time_range`.
- `CycleAgeing` data may be sparse: a NaN in a target column means the variable
  was not measured at that cycle, and that row is left out for that variable
  only, rather than every variable having to be measured at every cycle.

### Changed
- `analysis` and `time_range` are now reserved keys of a dict data input, so a
  column literally named either is read as a payload key.

### Fixed
- A `time_range` combined with `first_step` / `last_step` narrows within the
  chosen steps, rather than shifting which steps are chosen.

## [0.27.0] - 2026-09-22

### Added
- `CycleAgeingOptions` takes `termination` and `use_cv`, matching the new
  `DataLoader.generate_experiment` arguments. Both apply only to a generated
  experiment; passing either alongside an explicit one raises.

### Fixed
- A `pybamm.Symbol` `fun` on a `Constraint` or `Penalty` now survives submission
  through the API. `to_config` emits the symbol as a pybamm JSON tree, and the
  schema rebuilds it on validation, where previously the serialized dict was
  rejected against `is-instance[Symbol]` — so a symbolic constraint or penalty
  was local-only.
- A `DataLoader` passed as a `CycleAgeingOptions` experiment is wrapped in the
  `"from data"` form its consumers dispatch on. An unwrapped reference was
  resolved by nobody and then rejected by the parser. This also lets the protocol
  come from a different measurement than the fitted data.
- An entry or `DirectEntry` config that names more than one source of parameters
  — `path`, `values`, `pipeline_id` — is rejected with an error naming the keys
  that clash, instead of silently picking one.

## [0.26.0] - 2026-09-15

### Breaking changes
- A field value with no wire form now raises when the schema is built,
  naming the field, instead of reaching the server as a `repr` string
  such as `"<pybamm...SPM object at 0x...>"`. Nested failures report the
  full path (`options: model:`). Convert such values to a number,
  string, list, dict, or pybamm object before putting them in a schema.
- `callbacks` and `cost_logger` hold live objects and now raise on
  `to_config()`. They were never wire-capable — previously they were
  embedded raw and failed later, more confusingly, in the API client's
  JSON encoding.
- `custom_parameters` is rejected by name; use `parameters` instead. The
  positional slot is retained so a positional caller gets an error rather
  than silently shifting `constraints` into the vacated position.

### Changed
- Parameter serialisation moved onto the type: `ParameterValuesLike`
  carries the serialiser, so it applies on both construction and
  assignment for every field that uses it, rather than being re-declared
  per owner. This also fixes `custom_parameters` mappings never having
  their `citations` dropped or scalars flattened.
- Re-serialising an already-serialised parameter mapping now
  short-circuits instead of deep-walking the expanded symbol tree, so a
  read-modify-write on `.parameters` no longer pays the full cost.

### Removed
- The internal pybamm-model serialiser, superseded by pybamm's own
  `BaseModel.to_config()`. The old helper reported any non-lithium-ion
  model as `"custom"`; pybamm's emits the real module.
