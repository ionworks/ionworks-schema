...
def test_linked_pybamm_expression_serialises_to_symbol_json():
    """A parameter may be a pybamm expression linking other parameters.

    The pipeline parser reads these back with ``convert_symbol_from_json``, so
    they must leave as symbol-JSON rather than as a live pybamm object.
    """
    expr = pybamm.Parameter("a") + pybamm.Parameter("b")
    fit = iws.DataFit(
        objectives={},
        parameters={
            "P": iws.Parameter("P", initial_value=1.0, bounds=(0.0, 2.0)),
            "Linked": expr,
        },
    )
    assert isinstance(fit.to_config()["parameters"]["Linked"], dict)
