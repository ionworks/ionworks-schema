...
def test_objective_rejects_renamed_custom_parameters():
    """``custom_parameters`` was renamed; say so rather than silently ignoring it."""
    with pytest.raises(ValueError, match="renamed to 'parameters'"):
        iws.objectives.CurrentDriven(
            data_input="file:cycle_1C.csv",
            options={"model": "SPM"},
...
def test_unserialisable_value_raises_naming_its_field():
    """A value with no wire form must not survive ``to_config``.

    Returned as-is it reaches ``json.dumps`` in the caller's submission code,
    where ``default=str`` turns it into a ``repr`` the server cannot read — a
    config that looks fine locally and fails only server-side.
    """
    obj = iws.objectives.CurrentDriven(
        data_input="file:cycle_1C.csv", options={"model": "SPM"}
    )
    obj.options.model = object()

    with pytest.raises(ValueError, match=r"model: object has no wire form"):
        obj.to_config()


def test_parameters_serialise_on_assignment_not_just_construction():
    """``validate_assignment`` is on, so a reassigned mapping must serialise too."""
    obj = iws.objectives.CurrentDriven(
        data_input="file:cycle_1C.csv",
        options={"model": "SPM"},
        parameters={"x": pybamm.Scalar(2.0)},
    )
    obj.parameters = {"x": pybamm.Scalar(5.0)}
    assert obj.to_config()["parameters"] == {"x": 5.0}


def test_null_parameter_rejected_on_assignment():
    """Assignment gets the same null rejection as construction."""
    obj = iws.objectives.CurrentDriven(
        data_input="file:cycle_1C.csv", options={"model": "SPM"}, parameters={"x": 1.0}
    )
    with pytest.raises(ValidationError, match="'x' is set to None"):
        obj.parameters = {"x": None}
