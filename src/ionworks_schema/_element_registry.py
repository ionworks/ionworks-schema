...
#: Legacy display labels / old spellings → canonical wire discriminator. The
#: contract owns this vocabulary so the backend and parsers don't each copy it.
LEGACY_ELEMENT_TYPE_ALIASES: dict[str, str] = {
    "Direct Entry": "entry",
    "Data Fit": "data_fit",
    ...
    "Linear Confidence Interval": "linear_confidence_interval",
    "Sobol Sensitivity": "sobol_sensitivity",
}

CANONICAL_WIRE_VALUES: frozenset[str] = frozenset(LEGACY_ELEMENT_TYPE_ALIASES.values())


def canonicalize_element_type(raw: str) -> str:
    """Map a legacy element-type label to the canonical wire discriminator.

    Parameters
    ----------
    raw : str
        Element type as stored in a pipeline config. May be a canonical wire
        value (``entry``, ``data_fit``, …) or a legacy display label
        (``"Direct Entry"``, ``"Data Fit"``, …).

    Returns
    -------
    str
        Canonical wire value accepted by element validation.

    Raises
    ------
    ValueError
        If ``raw`` is not a string, or is neither a known wire value nor a
        known alias.
    """
    # Without this, an unhashable value raises TypeError from the lookups below
    # rather than the documented ValueError.
    if not isinstance(raw, str):
        raise ValueError(f"element_type must be a string; got {type(raw).__name__}.")
    canonical = LEGACY_ELEMENT_TYPE_ALIASES.get(raw)
    if canonical is not None:
        return canonical
    if raw in CANONICAL_WIRE_VALUES:
        return raw
    raise ValueError(
        f"Unknown element_type {raw!r}. Expected one of "
        f"{sorted(CANONICAL_WIRE_VALUES)} or legacy aliases "
        f"{sorted(LEGACY_ELEMENT_TYPE_ALIASES)}."
    )
...
def schema_class_for(element_type: str, type_name: str | None = None):
    ...
    if element_type == "entry":
        return None
    if element_type == "linear_confidence_interval":
        from .analysis import LinearConfidenceInterval

        return LinearConfidenceInterval
    if element_type == "sobol_sensitivity":
        from .analysis import SobolSensitivity
...
def _validate_entry(data: dict):
    ...
    named = [k for k in ("path", "values", "pipeline_id") if data.get(k) is not None]
    if len(named) > 1:
        raise ValueError(
            "An entry takes its parameters from one place, but this config sets "
            + " and ".join(repr(k) for k in named)
