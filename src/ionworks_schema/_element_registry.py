...
def validate_element(value):
    ...
    validated against it. An existing ``BaseSchema`` instance passes through
    untouched — this preserves ``ArrayDataFit`` (which has no serialized marker
    and is unreachable from a dict). A builtin scalar/collection (``None``,
    ``int``, ``str``, ``list``, ...) is never a valid element and is rejected,
    so a malformed element value can't slip past validation.
    """
