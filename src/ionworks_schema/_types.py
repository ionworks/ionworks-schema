...
if TYPE_CHECKING:
    from ionworksdata import AnalysisLoader, DataLoader
    import polars as pl
else:
    try:
    ...
    try:
        from ionworksdata import AnalysisLoader, DataLoader  # type: ignore[no-redef]
    except ImportError:  # pragma: no cover

        class DataLoader:  # type: ignore[no-redef]
            pass

        class AnalysisLoader:  # type: ignore[no-redef]
            pass
...
def _route_measurement_input(value: Any) -> Any:
    ...
    ``data``, ``options``, ``time_range``, ``analysis`` and ``metadata`` are
    therefore reserved — a column named one of these is read as a payload key.
...
- ``ionworksdata.AnalysisLoader``: one of a measurement's stored analyses,
  read as a feature table rather than a trace.
- ``dict``: routed by its keys to one of three named shapes —
  ...
  ``data``, ``options``, ``time_range``, ``analysis`` and ``metadata`` are
  reserved keys: a column literally named one of these is read as a payload
  key instead, not as a column.
"""

ParameterValuesLike = Annotated[
    dict[str, Any] | pybamm.ParameterValues, BeforeValidator(serialize_parameters)
]
"""A raw ``{name: value}`` mapping or a ``pybamm.ParameterValues``.

Serialised on the way in, so a plain dict and a ``pybamm.ParameterValues``
cannot diverge and the stored value is wire-native however it was assigned.
"""
