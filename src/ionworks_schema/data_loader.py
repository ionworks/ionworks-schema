...
class TimeRangeSpec(BaseSchema):
    ...
    Bounds are ``Time [s]`` values, never wall-clock: ``start_time`` is
    editable metadata a pin must not silently re-anchor to. Give the far bound
    as ``end`` or ``duration``, which resolves to ``end`` at construction. Step
    selection comes first: ``first_step`` picks from the whole measurement, and
    the window narrows within the steps it chose.
    ...
    start : float
        ...
        Inclusive upper bound, in seconds. Must not precede *start*. Supply
        this or *duration*, not both.
    duration : float
        Seconds of data to use, measured from *start*. Input only: it sets
        *end* and is not itself part of the window.

    Examples
    --------
    >>> TimeRangeSpec(start=0.0, end=3600.0).to_config()
    {'start': 0.0, 'end': 3600.0}

    The first day of a test, however long that test eventually runs:

    >>> TimeRangeSpec(start=0.0, duration=86_400.0).to_config()
    {'start': 0.0, 'end': 86400.0}
    """
    ...
    @classmethod
    def resolve_duration(cls, data):
        """Turn a ``duration`` into the ``end`` the wire format carries.

        Resolved before validation rather than kept as a field so ``duration``
        never reaches the config: consumers read ``start``/``end`` and reject
        anything else, and a window storing both could disagree with itself
        after an assignment.
        """
        if not isinstance(data, dict) or "duration" not in data:
            return data
        rest = {k: v for k, v in data.items() if k != "duration"}
        duration = data["duration"]
        if "end" in rest:
            raise ValueError(
                "time_range takes either end or duration, not both; got "
                f"end={rest['end']} and duration={duration}"
            )
        if isinstance(duration, bool) or not isinstance(duration, (int, float)):
            raise ValueError(f"time_range duration must be a number; got {duration!r}")
        if duration < 0:
            raise ValueError(
                f"time_range duration must not be negative; got {duration}"
            )
        start = rest.get("start", 0.0)
        if isinstance(start, bool) or not isinstance(start, (int, float)):
            # Leave it; the field validator reports the bad start itself.
            return data
        return {**rest, "end": float(start) + float(duration)}
...
class AnalysisSpec(BaseSchema):
    """Selector naming one analysis already stored on a measurement.

    An analysis is a stored artifact, not something computed on demand: a row in
    the platform's analyses table plus a parquet of extracted features. This
    selector says which of a measurement's analyses to read as the fit's target
    data. Every field given must match, so two analyses sharing a type are
    separated by name.

    Parameters
    ----------
    analysis_id : str, optional
        The analysis' own id. Exact, and the form a resolved run records, so a
        re-run cannot drift onto a different analysis.
    ...
    Examples
    --------
    >>> iws.AnalysisSpec(analysis_type="lam_lli_from_rpt").to_config()
    {'analysis_type': 'lam_lli_from_rpt'}
    """

    # Merged into a payload dict the consumer validates by exact keys.
    _emit_type: bool = False

    analysis_id: str | None = Field(
        default=None, description="The analysis' own id; exact."
    )
    analysis_type: str | None = Field(
        default=None, description="Kind of analysis, e.g. 'lam_lli_from_rpt'."
    )
    name: str | None = Field(
        default=None, description="The analysis' name, as created."
    )

    @model_validator(mode="after")
    def validate_selector_is_not_empty(self):
        """Reject a selector that would match every analysis on the measurement."""
        if not (self.analysis_id or self.analysis_type or self.name):
            raise ValueError(
                "analysis must give at least one of analysis_id, analysis_type "
                "or name; an empty selector matches every analysis on the "
                "measurement"
            )
        return self
...
class DataPayloadSpec(BaseSchema):
    ...
    time_range : TimeRangeSpec, optional
        ...
        extend.
    analysis : AnalysisSpec, optional
        Read one of the measurement's stored analyses as the data, rather than
        its time series. Cannot be combined with ``options`` or ``time_range``,
        neither of which can apply to a feature table.
    """
    ...
    analysis: AnalysisSpec | None = Field(
        default=None,
        description=(
            "Read one of the measurement's stored analyses as the data, rather "
            "than its time series."
        ),
    )

    @model_validator(mode="after")
    def validate_analysis_stands_alone(self):
        """Reject options that cannot apply to an analysis table.

        ``options`` are ``DataLoader`` options, and an analysis resolves to a
        bare frame that never reaches a ``DataLoader``, so they would be
        silently ignored. ``time_range`` is a window of elapsed seconds into a
        time series, which a per-cycle feature table does not have.
        """
        if self.analysis is None:
            return self
        conflicts = [
            field
            for field in ("options", "time_range")
            if getattr(self, field) is not None
        ]
        if conflicts:
            raise ValueError(
                f"{' and '.join(conflicts)} cannot be combined with 'analysis': "
                "an analysis is read as a feature table, not loaded as a time "
                "series, so neither would have any effect"
            )
        return self
