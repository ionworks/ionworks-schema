...
def serialize_parameter_value(name: str = "", value: Any = None) -> Any:
...
def reject_null_parameter_values(parameters) -> None:
    ...
    For a ``parameters`` mapping whose values are fit specs rather than pybamm
    values (``DataFit``), so it is never serialised and never reaches
    :func:`serialize_parameter_value`, which rejects ``None`` itself.
