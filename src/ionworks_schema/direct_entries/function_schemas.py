...
class ArrheniusButlerVolmerExchangeCurrentDensity(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.arrhenius_butler_volmer_exchange_current_density``.
...
class ArrheniusParticleDiffusivity(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.arrhenius_particle_diffusivity``. Produces
    a function of stoichiometry and temperature, leaving the reference
...
class AverageOcp(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.average_ocp``. Produces a single OCP
...
class ConstantElectrolyte(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.constant_electrolyte``. Sets the initial
    electrolyte concentration to the supplied value.
    """
...
class NymanElectrolyte(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.nyman_electrolyte``. Supplies concentration-
    dependent electrolyte diffusivity and conductivity from Nyman et al.
...
class ArrheniusElectrolyteDiffusivity(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.arrhenius_electrolyte_diffusivity``. Leaves
    the reference diffusivity and activation energy as free parameters (no
...
class ArrheniusElectrolyteConductivity(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.arrhenius_electrolyte_conductivity``.
...
class LandesfeindElectrolyte(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.landesfeind_electrolyte``. Supplies
    concentration- and temperature-dependent conductivity, diffusivity,
...
class Temperatures(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.temperatures``. Useful for isothermal
    simulations where all three temperature parameters share the same
    value.
    """
...
class ZeroEntropicChange(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.zero_entropic_change``. Useful when
    entropic heating is ignored or the data needed to parameterise it is
...
class ZeroActivationEnergy(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.zero_activation_energy``. Disables the
...
class FromJson(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.from_json``. Reads a JSON file of
...
class StandardDefaults(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.standard_defaults``. Sets
...
class OneCm2Cell(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.one_cm2_cell``. Sets ``Electrode height
...
class Bruggeman(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.bruggeman``. Sets the Bruggeman exponents
...
class SpmDefaults(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.spm_defaults``. Sets parameters (e.g.
...
class LithiumMetalAnode(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.lithium_metal_anode``. Supplies parameter
...
class SeiDefaults(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.sei_defaults``. Provides the SEI kinetic
...
class LiPlatingDefaults(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.li_plating_defaults``. Provides plating /
...
class MechanicalDegradationDefaults(DirectEntryFunctionSchema):
    ...
    Mirrors ``iwp.direct_entries.mechanical_degradation_defaults``.
