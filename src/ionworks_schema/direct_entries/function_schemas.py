"""Light wrapper schemas for ``iws.direct_entries`` functions.

Each schema validates args/kwargs and serializes to {"name": <name>, **kwargs}
— except ``FromJson``, which carries the values it reads. Use via
``iws.direct_entries.<snake_case_name>(...)``.
"""
...
class DirectEntryFunctionSchema(BaseSchema):
    """Base wrapper for named ``iws.direct_entries`` helper functions.

    Subclasses set ``__function_name__`` (the snake_case function
...
class FromJson(DirectEntryFunctionSchema):
    ...
    Reads a JSON file of ``{parameter_name: value}`` pairs and returns a
    ``DirectEntry`` with those values. pybamm-serialised symbols are rehydrated
    automatically.
    """
    ...
    file_path: str = Field(
        ...
        description=(
            ...
            "parameter values. Read when the config is built, so the values "
            "travel with it."
        ),
    )
    ...
    def to_config(self) -> dict:
        ...
        return DirectEntry(
            ...
            source=f"Parameters read from JSON file: {self.file_path}",
        ).to_config()
