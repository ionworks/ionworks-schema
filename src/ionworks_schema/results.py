...
class ConfidenceIntervalResult(BaseResults):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._intervals = {k: dict(v) for k, v in (intervals or {}).items()}
    ...
    def intervals(self) -> dict:
        return {k: dict(v) for k, v in self._intervals.items()}
    ...
    def bounds_for(self, name: str) -> tuple[float, float]:
        entry = self._intervals[name]
        return float(entry["lower"]), float(entry["upper"])

    def width(self, name: str) -> float:
        lower, upper = self.bounds_for(name)
        return upper - lower
    ...
    def to_config(self) -> dict:
        return {
            **super().to_config(),
            ...
            "intervals": {k: dict(v) for k, v in self._intervals.items()},
            "covariance": self._covariance,
        }


class SensitivityResult(BaseResults):
    ...
    def __init__(
        ...
        *,
        ...
        total_order: dict | None = None,
        first_order_ci: dict | None = None,
        total_order_ci: dict | None = None,
        second_order: dict | None = None,
        second_order_ci: dict | None = None,
        evaluation_stats: dict | None = None,
        n_samples: int = 0,
        ...
        **kwargs,
        ...
        super().__init__(parameter_values=parameter_values, **kwargs)
        self._method = method
        self._first_order = dict(first_order or {})
        self._total_order = dict(total_order or {})
        self._first_order_ci = None if first_order_ci is None else dict(first_order_ci)
        self._total_order_ci = None if total_order_ci is None else dict(total_order_ci)
        self._second_order = None if second_order is None else dict(second_order)
        self._second_order_ci = (
            None if second_order_ci is None else dict(second_order_ci)
        )
        self._evaluation_stats = dict(evaluation_stats or {})
        self._n_samples = int(n_samples)
        self._log_transformed = bool(log_transformed)
    ...
    @property
    def total_order(self) -> dict:
        return self._total_order

    @property
    def first_order_ci(self) -> dict | None:
        return self._first_order_ci

    @property
    def total_order_ci(self) -> dict | None:
        return self._total_order_ci

    @property
    def second_order(self) -> dict | None:
        return self._second_order

    @property
    def second_order_ci(self) -> dict | None:
        return self._second_order_ci

    @property
    def evaluation_stats(self) -> dict:
        return self._evaluation_stats
    ...
    def _sorted_contributors(self, kind: str = "total") -> list[tuple[str, float]]:
        source = self._total_order if kind == "total" else self._first_order
        return sorted(source.items(), key=lambda kv: kv[1], reverse=True)

    def top_contributors(self, n: int = 5, kind: str = "total"):
        return self._sorted_contributors(kind)[:n]

    def bottom_contributors(self, n: int = 5, kind: str = "total"):
        return self._sorted_contributors(kind)[-n:]
    ...
    def to_config(self) -> dict:
        return {
            **super().to_config(),
            ...
            "first_order_ci": self._first_order_ci,
            "total_order_ci": self._total_order_ci,
            "second_order": self._second_order,
            "second_order_ci": self._second_order_ci,
            "evaluation_stats": self._evaluation_stats,
