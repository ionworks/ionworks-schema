...
class MultiCost(ErrorFunction):
    ...
    ``calculation_structure`` accepts only ``None`` for every value here. A
    MultiCost only weights and sums what its components return, so per-variable
    scoping belongs on the component costs. Nothing enforces that at
    construction: building this object with a variable list succeeds, and what
    happens next depends on how the configuration is used. Converting it
    directly to a runtime cost raises; sending it as a stored configuration
    drops the variable list and logs a warning, because a configuration on the
    wire carries nothing to distinguish one written today from one written a
    year ago.
