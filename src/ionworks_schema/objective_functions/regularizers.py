...
import pybamm
from pybamm.expression_tree.operations.serialise import convert_symbol_from_json
...
class Regularizer(BaseSchema):
    ...
    @field_validator("fun", mode="before", check_fields=False)
    @classmethod
    def _fun_from_wire(cls, value):
        # to_config emits a symbol as the pybamm JSON tree; rebuild it so a submitted
        # config re-validates instead of failing is-instance[Symbol].
        return convert_symbol_from_json(value) if isinstance(value, dict) else value
...
def resolve_prior(value, name=None):
    ...
    so a live runtime prior must be converted before it enters a schema, not
    embedded raw.
    """
    if isinstance(value, Prior):
        return value
    if not isinstance(value, dict):
        reject_runtime_object(value, "prior", "a Prior instance")
    data = dict(value)
