...
class Constraint(Regularizer):
    """Fit-time constraint; API requests require a numeric ``fun``.
...
class Penalty(Regularizer):
    ...
    Notes
    -----
    As for :class:`Constraint`, a ``pybamm.Symbol`` ``fun`` is local-only: it does
    not survive submission through the API, where server-side validation rejects the
    serialized symbol dict against ``is-instance[Symbol]``.
    """
