...
from ..serialize import reject_null_parameter_values
...
# ``cost_logger`` accepts a runtime cost logger on the execution path;
# the schema package doesn't model the cost logger itself.
...
class DataFit(BaseSchema):
    ...
    parameters : dict[str, Parameter | pybamm.Symbol | callable] | None, optional
        ...
        - a pybamm expression, in which case other referenced
          parameters must also be supplied as ``iws.Parameter`` objects
        ...
        from the ``iws.Parameter`` reference.
    ...
    # Values are ``iws.Parameter`` instances, pybamm expressions, or
    # callables — heterogeneous and runtime-validated.
    parameters: dict[str, Any] | None = Field(
        default=None,
        description=(
            "Mapping of parameter name to the quantity being fitted. Values "
            "may be ``iws.Parameter`` instances, ``pybamm`` expressions "
            "referencing other ``iws.Parameter`` entries, or Python "
    ...
    cost_logger: _CostLoggerLike | None = Field(
        ...
        description=(
            ...
            "during the fit. Defaults to a cost logger with default "
            "options."
        ),
    )
    ...
    def __init__(
        ...
        **subclass_fields,
        ...
        # `**subclass_fields` forwards fields declared only on a DataFit subclass
        # (e.g. `confidence_level`) so they need no `__init__` override.
        super().__init__(
