...
def _resolve_distribution(value, handler):
    ...
    objects (a config dict or a ``Distribution`` instance) — a live runtime
    distribution must be converted before it enters a schema, not embedded raw.
    """
    ...
    reject_runtime_object(value, "distribution", "a Distribution instance")
