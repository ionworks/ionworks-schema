...
class BaseSchema(BaseModel):
    ...
    discriminator and skips the schema's field-name mappings, so the
    dict it returns is not what the Ionworks API expects. Always use
    ``.to_config()`` to build a payload.
    """
    ...
    def to_config(self) -> dict:
        ...
        does not apply the schema's field-name mappings, so it produces a
        different dict than ``.to_config()`` — one the API may reject. For
        example, ``iws.stats.Uniform(lb=0.0, ub=1.0).to_config()`` yields
