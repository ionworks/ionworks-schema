...
def reject_runtime_object(value: Any, field_name: str, expected: str) -> NoReturn:
    ...
    objects — a config dict or the matching schema instance — so a live
    ``ionworkspipeline`` runtime object must be converted (via ``.to_config()``
    or the matching ``iws`` schema class) before it enters a schema, rather than
    embedded raw. Callers invoke this on the fall-through branch after they have
    already accepted the dict and schema-instance forms.
    ...
    raise ValueError(
        ...
        f"got {type(value).__name__}. Live ionworkspipeline runtime objects are "
...
class BaseSchema(BaseModel):
    ...
    def to_config(self) -> dict:
        ...
        for key, value in self:
            ...
            if output_key == "data" and _is_dataframe(value):
