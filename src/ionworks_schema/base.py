...
class Pipeline(BaseSchema):
    ...
    A pipeline is a sequence of pipeline elements (``DataFit``,
    ``DirectEntry``, ``Validation``, ``Calculation``, …). The output
    parameters of one element become the input of the next, and the
    collected ``to_config`` output is submitted to the Ionworks API for
    execution.
    """
