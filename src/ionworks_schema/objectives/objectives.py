...
def _resolve_objective(value, handler):
    ...
    if isinstance(value, BaseObjective):
        return handler(value)
    raise ValueError(
        f"Invalid objective {value!r}: expected an objective config dict or an "
        f"ionworks_schema objective instance; got {type(value).__name__}. Live "
        f"ionworkspipeline runtime objects are not accepted inside a schema — "
        f"pass a config dict or the matching ionworks_schema objective."
    )
