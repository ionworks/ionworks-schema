...
from pydantic import Field, WrapValidator, field_validator
...
from ..serialize import serialize_parameters
...
def _resolve_objective(value, handler):
    ...
    objects — a config dict or a schema objective instance. A live
    ``ionworkspipeline`` runtime objective must be converted (via
    ``.to_config()`` or the matching ``iws`` schema class) before it enters a
    schema, rather than embedded raw.
    """
