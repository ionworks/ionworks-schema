...
class SimulationObjective(FittingObjective):
    ...
    def _require_a_model(self):
        ...
        if options.get("model") is None and not options.get("parameterized_model_id"):
            raise ValueError(
...
class CalendarAgeing(SimulationObjective):
    ...
    options: dict[str, Any] | None = Field(
        ...
        description=(
            ...
            "to the simulation; may include ``solver_kwargs`` to tune the "
...
class CurrentDriven(SimulationObjective):
    ...
    options : dict, optional
        Current-driven settings:
            ...
            * ``interpolant_lossless`` (bool): when True, use the current samples
                directly for the interpolant (reproducing the input exactly)
                instead of compressing within ``interpolant_atol`` /
                ``interpolant_rtol``, which are then ignored. Default ``False``.
    ...
    options: dict[str, Any] | None = Field(
        ...
        description=(
            ...
            "forwarded to the simulation), ``objective variables`` (list of "
            "variables to fit; default depends on ``independent variable``), "
            "``interpolant_atol`` / ``interpolant_rtol`` (tolerances for the "
            "current interpolant), and ``interpolant_lossless`` (bool; use the "
            "samples directly instead of compressing)."
        ),
    )
...
class CycleAgeing(SimulationObjective):
    ...
    options: dict[str, Any] | None = Field(
        ...
        description=(
            ...
            "``simulation_kwargs`` (kwargs forwarded to the simulation; may include "
...
class EIS(SimulationObjective):
    ...
    options : dict, optional
        EIS settings:
            ...
            * ``simulation_kwargs`` (dict): kwargs forwarded to the EIS
                simulator (:class:`pybamm.EISSimulation`). Only the mesh /
                discretisation kwargs it accepts — ``var_pts``,
                ``submesh_types``, ``geometry``, ``spatial_methods`` — are
                applied, so a refined particle mesh can be passed here just as
                for the time-domain objectives. Any time-domain-only key
                (``solver``, ``solver_kwargs``, ``output_variables``,
                ``solve_kwargs``, ``experiment``, …) is ignored, so a
                ``simulation_kwargs`` dict shared with a ``CurrentDriven`` /
                ``Pulse`` objective in the same fit is accepted without error.
                Defaults to ``None``.
    callbacks, constraints, penalties, parameters
        Shared with :class:`BaseObjective`.
    ...
    options: dict[str, Any] | None = Field(
        ...
        description=(
            ...
            "``pybamm.EISSimulation`` — only the mesh/discretisation kwargs it "
            "accepts, ``var_pts`` / ``submesh_types`` / ``geometry`` / "
            "``spatial_methods``, are applied; time-domain-only keys such as "
            "``solver`` / ``output_variables`` / ``solve_kwargs`` are ignored)."
        ),
    )
    callbacks: Any | None = Field(default=None, description=_CALLBACKS_DESCRIPTION)  # noqa: ANN401
    custom_parameters: dict[str, Any] | None = Field(
...
class MSMRHalfCell(FittingObjective):
    ...
    options : dict, optional
        MSMR half-cell settings:
            ...
            * ``constrain Xj method``: how to constrain the Xj parameters to
                sum to 1 — ``"project"`` (default; complete Xj groups are
                projected onto the bounded simplex each evaluation, giving an
                exact sum), ``"explicit"`` (a soft equality constraint, can
                drift), or ``"reformulate"`` (eliminates the last Xj by
                substitution). The default changed from ``"explicit"`` to
                ``"project"``; configs omitting this option now use
                ``"project"``. Only used with the ``"Xj"`` species format.
    ...
    options: dict[str, Any] | None = Field(
        ...
        description=(
            ...
            "default False), ``constrain Xj method`` (``'project'``, "
            "``'explicit'``, or ``'reformulate'``; default ``'project'``; "
            "only used with ``'Xj'`` species format), and "
            "``preserve U0j order`` (bool — "
...
class Pulse(SimulationObjective):
    ...
    options: dict[str, Any] | None = Field(
        ...
        description=(
            ...
            "the simulation; default None), ``objective variables`` "
            ...
            "if a solver is supplied, otherwise ``1e-6`` / ``1e-4``), and "
            "``interpolant_lossless`` (bool; use the samples directly instead "
            "of compressing)."
        ),
    )
    _validate_options = field_validator("options")(_reject_interactive_preprocessing)
