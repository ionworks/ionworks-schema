    ...
    options : dict, optional
        Calendar-ageing settings:
            ...
            * ``simulation_kwargs`` (dict): kwargs forwarded to the
                simulation. May include ``solver_kwargs`` to tune the auto-built solver,
                e.g. ``{"options": {"compile": True}}`` to enable compilation.
                Ignored if an explicit ``solver`` is also provided.
                Defaults to ``None``.
    ...
    options : dict, optional
        Current-driven settings:
            ...
            * ``interactive_preprocessing`` (bool): if ``True``, adjusts
                ``interpolant_atol``/``rtol`` automatically from the data.
                Default ``False``.
...
class ElectrodeBalancing(FittingObjective):
    ...
    options: dict[str, Any] | None = Field(
        ...
        description=(
            ...
            "``dUdQ cutoff`` (float — ignore data points with very large "
            "``dU/dQ``, typically near the voltage limits; default None) "
            "and ``direction`` (``'charge'`` or ``'discharge'`` — direction "
...
class Pulse(SimulationObjective):
    ...
    options: dict[str, Any] | None = Field(
        ...
        description=(
            ...
            "``interactive_preprocessing`` (bool — adjust interpolant "
            "tolerances based on data; default False), and "
