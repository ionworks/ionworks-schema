...
class Material(BaseModel):
    ...
    Holds a named parameter set — the three thermodynamic host-site
    fields for MSMR-style materials, or any other pybamm-compatible
    parameter values for the material.

    Parameters
    ----------
    name : str
        Human-readable material identifier (e.g.
        ``"NMC - Verbrugge 2017"``). Used as the lookup key in the
        library.
    description : str, optional
        Free-text description of the material — typically includes the
        citation or source the parameter values were taken from.
    parameter_values : dict, optional
        Mapping of pybamm parameter names to their values for this
        material. For MSMR materials typically contains the host-site
        standard potentials, occupancy fractions, and ideality factors.
    """

    # ``Material`` subclasses ``BaseModel`` (not ``BaseSchema``), so the default
    # is ``extra="ignore"`` — set ``forbid`` explicitly so unknown sibling fields
    # are rejected (``parameter_values`` stays the open pybamm parameter set).
    model_config = ConfigDict(extra="forbid")

    name: str = Field(
        ...,
        description=(
            "Human-readable material identifier (e.g. ``'NMC - Verbrugge "
            "2017'``). Used as the lookup key in the library."
        ),
    )
...
class Library(BaseModel):
    ...
    Exposes the bundled set of reference materials (e.g. graphite,
    NMC, LFP) via static lookups by name.
    """
