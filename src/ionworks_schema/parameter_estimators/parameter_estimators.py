...
class BayesianOptimizationOptions(_AlgorithmOptions):
    ...
    defaults set by the Ionworks pipeline. ``extra="allow"`` keeps forward
    compatibility for option keys added later.
...
class SOBEROptions(_AlgorithmOptions):
    ...
    Ionworks pipeline. ``extra="allow"`` keeps forward compatibility for
    option keys added later.
...
class CMAES(_PassthroughOptimizer):
...
class DifferentialEvolution(_PassthroughOptimizer):
...
class BayesianOptimization(_PassthroughOptimizer):
...
class SOBER(_PassthroughOptimizer):
...
class TuRBO(_PassthroughOptimizer):
...
class DummyOptimizer(_PassthroughOptimizer):
...
class DummySampler(_PassthroughOptimizer):
...
class PSO(_PassthroughOptimizer):
...
class AskTellOptimizer(BaseSchema):
    ...
    **Choosing a method.** ``"CMAES"`` (the default) is a robust general-purpose
    ...
    surrogate_convergence_tol: float | int | None = Field(
        ...
        description=(
            "Relative tolerance on the model-native convergence signal for "
            "surrogate algorithms (BO, SOBER). Fires when expected improvement "
            "at the best candidate is below ``tol`` times the range of observed "
            "costs, for ``convergence_patience`` consecutive iterations. "
            "Default 1e-3. Set to ``None`` to disable."
        ),
    )
    algorithm_options: (
...
class PointEstimateOptimizer(_PassthroughOptimizer):
...
class PointEstimateSampler(_PassthroughOptimizer):
...
class XNES(_PassthroughOptimizer):
