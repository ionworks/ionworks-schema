...
#: see them. A ``None`` value means the optimizer computes the default
...
class Nested(BaseSchema):
    """Bi-level optimizer: an outer ``optimizer`` over ``parameters`` wrapping an
    ``inner`` over the remaining parameters, which is run to optimality at each
    outer point (variable projection when ``inner`` is least-squares). ``inner``
    may itself be a ``Nested`` for deeper nesting.

    Examples
    --------
    >>> opt = iws.parameter_estimators.Nested(
    ...     parameters=["a", "b"],
    ...     optimizer=iws.parameter_estimators.CMAES(),
    ...     inner=iws.parameter_estimators.ScipyLeastSquares(),
    ... )
    """

    type: Literal["Nested"] = "Nested"
    parameters: list[str] = Field(
        description="Fit parameters this level's `optimizer` controls; the rest go to `inner`.",
    )
    optimizer: "OptimizerUnion" = Field(
        description="Outer optimizer searching the `parameters` subset.",
    )
    inner: "OptimizerUnion" = Field(
        description="Inner optimizer for the remaining parameters; may be another `Nested`.",
    )

    @model_validator(mode="after")
    def _reject_samplers(self) -> "Nested":
        # Nested runs each sub-estimator to a point optimum; samplers return a
        # posterior, not a point, so they cannot occupy either slot. Catch at
        # construction rather than failing obscurely at run time.
        for field, est in (("optimizer", self.optimizer), ("inner", self.inner)):
            if isinstance(est, _SAMPLER_TYPES):
                raise ValueError(
                    f"Nested.{field} must be an optimizer, not a sampler "
                    f"({type(est).__name__}); samplers cannot be nested."
                )
        return self
...
class AskTellOptimizer(_AskTellBase):
    ...
    **Choosing a method.** ``"CMAES"`` (the default) is a robust general-purpose
    ...
    # "Pints" is this class's legacy wire tag: the runtime resolves it to an
    # ask/tell optimizer, so the contract must read it the same way.
    type: Literal["AskTellOptimizer", "Pints"] = "AskTellOptimizer"
    method: str = Field(
        default="CMAES",
        description=(
...
class PintsSampler(BaseSchema):
    ...
    type: Literal["PintsSampler"] = "PintsSampler"
    method: str = Field(
        default="DramACMC",
        description=(
...
class ScipyLsqLinear(_PassthroughOptimizer):
    """Bounded linear least-squares via scipy.optimize.lsq_linear.

    Solves a bounded least-squares problem whose residual is linear in the
    fitted parameters, reaching the bounded optimum in a single solve (no
    Gauss-Newton iteration). Intended as the inner solver of a variable-
    projection / nested fit whose inner parameters enter the model linearly; for
    nonlinear residuals use ``ScipyLeastSquares``.

    Parameters
    ----------
    method : str, optional
        ``scipy.optimize.lsq_linear`` method: ``'bvls'`` (default) or ``'trf'``.
    jac_rel_step : float, optional
        Relative step for the finite-difference design matrix (default ``1e-3``);
        larger than a derivative step since the residual is assumed linear.
    warm_start : bool, optional
        Reuse the previous solve's active set (default True) so the inner solve
        stays consistent across outer perturbations and the concentrated objective
        stays smooth; falls back to a cold solve when it is no longer optimal.
    `**kwargs`
        Additional arguments passed to ``scipy.optimize.lsq_linear`` (e.g. ``tol``,
        ``max_iter``). See scipy documentation for complete options."""

    type: Literal["ScipyLsqLinear"] = "ScipyLsqLinear"
...
# ponytail: explicit tuple — the concrete samplers have no shared ``Sampler``
# base to isinstance on, so a new sampler must be added here too. Used by
# ``Nested._reject_samplers``.
_SAMPLER_TYPES = (GridSearch, DummySampler, PointEstimateSampler, PintsSampler)
...
# Nested.optimizer / .inner are typed OptimizerUnion (defined above), a forward
# reference because Nested is itself a union member.
