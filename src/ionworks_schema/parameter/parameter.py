...
class Parameter(BaseSchema):
    ...
    name: Any = Field(
        ...,
        description=(
            "Name of the parameter. Supplied by the parser's dict key when the "
            "parameter appears inside a ``parameters`` mapping, and excluded "
    ...
    prior: Any | None = Field(
        default=None,
        description=(
            "Prior distribution for the parameter, used by regularized and "
            "Bayesian fits. Accepts an ``iwp.stats.Distribution`` or a nested "
            "``{'distribution': ..., 'regularizer_weight': ...}`` mapping."
        ),
    )
    normalize: Any | None = Field(
        default=None,
        description=(
    ...
    initial_guess_distribution: Any | None = Field(
        default=None,
        description=(
            "Distribution used to generate initial guesses for multistarted "
            "fits. Defaults to a uniform distribution between the bounds. "
            "Must be either ``iwp.stats.Uniform`` or ``iwp.stats.PointMass``."
        ),
    )
