...
# and skips field-name mappings, producing a dict the API may reject).
...
`to_config()` is the only supported serializer. Pydantic's `model_dump()` is not a wire format: it omits the `type` discriminator and does not apply the schema's field-name mappings, so it produces a different dict than `to_config()` — one the API may reject. Always build API payloads with `to_config()`.
